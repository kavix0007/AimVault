...
-- Images are stored only as external URLs in public.crosshairs.image_url.
-- Supabase Storage is intentionally NOT used. Do not create a storage bucket for AimVault.

-- Existing AimVault entries. Optional seed data; do not run this section again if the rows are already in your live database.
...
-- Pro player profiles for clean SEO URLs such as /tenz/ and /zekken/.
...
create index if not exists player_profiles_published_idx on public.player_profiles(published);
alter table public.player_profiles enable row level security;
drop policy if exists "public can read published player profiles" on public.player_profiles;
create policy "public can read published player profiles" on public.player_profiles for select to anon, authenticated using (published = true or public.is_admin());
drop policy if exists "admins can insert player profiles" on public.player_profiles;
create policy "admins can insert player profiles" on public.player_profiles for insert to authenticated with check (public.is_admin());
drop policy if exists "admins can update player profiles" on public.player_profiles;
create policy "admins can update player profiles" on public.player_profiles for update to authenticated using (public.is_admin()) with check (public.is_admin());
drop policy if exists "admins can delete player profiles" on public.player_profiles;
create policy "admins can delete player profiles" on public.player_profiles for delete to authenticated using (public.is_admin());
...
insert into public.player_profiles (name,slug,title,description,meta_description,published) values
('TenZ','tenz','TenZ Valorant Crosshair Code','TenZ Valorant crosshair code, preview, and related AimVault crosshair settings. Add your own unique notes here from the Admin dashboard.','TenZ Valorant crosshair code and settings. Preview and copy the crosshair on AimVault.',true),
('Zekken','zekken','Zekken Valorant Crosshair Code','Zekken Valorant crosshair code, preview, and related AimVault crosshair settings. Add your own unique notes here from the Admin dashboard.','Zekken Valorant crosshair code and settings. Preview and copy the crosshair on AimVault.',true),
('aspas','aspas','aspas Valorant Crosshair Code','aspas Valorant crosshair code, preview, and related AimVault crosshair settings. Add your own unique notes here from the Admin dashboard.','aspas Valorant crosshair code and settings. Preview and copy the crosshair on AimVault.',true),
('Demon1','demon1','Demon1 Valorant Crosshair Code','Demon1 Valorant crosshair code, preview, and related AimVault crosshair settings. Add your own unique notes here from the Admin dashboard.','Demon1 Valorant crosshair code and settings. Preview and copy the crosshair on AimVault.',true),
('nAts','nats','nAts Valorant Crosshair Code','nAts Valorant crosshair code, preview, and related AimVault crosshair settings. Add your own unique notes here from the Admin dashboard.','nAts Valorant crosshair code and settings. Preview and copy the crosshair on AimVault.',true)
