...
values ('crosshair-images','crosshair-images',true)
on conflict (id) do update set public=true;

drop policy if exists "public can view crosshair images" on storage.objects;
create policy "public can view crosshair images" on storage.objects
for select using (bucket_id = 'crosshair-images');

drop policy if exists "admins can upload crosshair images" on storage.objects;
create policy "admins can upload crosshair images" on storage.objects
for insert to authenticated with check (bucket_id='crosshair-images' and public.is_admin());

drop policy if exists "admins can update crosshair images" on storage.objects;
create policy "admins can update crosshair images" on storage.objects
for update to authenticated using (bucket_id='crosshair-images' and public.is_admin()) with check (bucket_id='crosshair-images' and public.is_admin());

drop policy if exists "admins can delete crosshair images" on storage.objects;
create policy "admins can delete crosshair images" on storage.objects
for delete to authenticated using (bucket_id='crosshair-images' and public.is_admin());
